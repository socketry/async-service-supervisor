...
describe Async::Service::Supervisor::UtilizationMonitor do
	...
	it "can register a worker" do
		...
		expect(status[:utilization_monitor]).to have_keys("test_service")
	end
	...
	it "handles worker without utilization schema" do
		...
		status = monitor.status
		expect(status[:utilization_monitor]).to be == {}
	end
	...
	it "can read utilization data from shared memory" do
		...
		# Read back the data
		status = monitor.status
		expect(status[:utilization_monitor]["test_service"]).to have_keys(
	...
	it "aggregates metrics by service name" do
		...
		status = monitor.status
		expect(status[:utilization_monitor]).to have_keys("test_service", "other_service")
		expect(status[:utilization_monitor]["test_service"]).to have_keys(
			connections_total: be == 100,
			connections_active: be == 5
		)
		expect(status[:utilization_monitor]["other_service"]).to have_keys(
	...
	it "handles worker without service name" do
		...
		status = monitor.status
		expect(status[:utilization_monitor]).to have_keys("unknown")
		expect(status[:utilization_monitor]["unknown"]).to have_keys(
	...
	it "handles removal of non-existent worker" do
		...
		expect(monitor.status[:utilization_monitor]).to be == {}
	end
	...
	it "frees segments when workers are removed" do
		...
		status = monitor.status
		expect(status[:utilization_monitor]["test_service"]).to be_a(Hash)
	end
	...
	it "automatically resizes when running out of segments" do
		...
		segments_per_page.times do |i|
			...
			if small_monitor.status[:utilization_monitor]["test_service"]
		...
		expect(status[:utilization_monitor]).to have_keys("test_service")
		service_data = status[:utilization_monitor]["test_service"]
